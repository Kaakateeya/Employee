(function() {
    ...
    angular
        .module('Kaakateeya')
        .factory('marketingModel', ['marketingservice', 'complex-slide-config',
            ...
            function(marketingservice, config, authSvc, helperservice, commonFactory, uibModal, alertss, modelpopupopenmethod,
                ...
                model.MarketingSlideShowBind = function(from, to) {
                    ...
                    var inputobj = {
                        ...
                        i_PageFrom: from,
                        i_PageTo: to,
                        dtFromProceedDate: $filter('date')(model.fromticketcreateddate, 'yyyy-MM-dd hh:mm:ss'),
                        dtToProceedDate: $filter('date')(model.toticketcreateddate, 'yyyy-MM-dd hh:mm:ss'),
                    ...
                    marketingservice.getMarketingSlideShowBind(inputobj).then(function(response) {
                        ...
                        if (response.data && response.data.Marketingslideticket !== null && response.data.Marketingslideticket.length > 0) {
                            ...
                            if (from === 1) {
                                config.slides = [];
                                config.setSlides(response.data.Marketingslideticket, 10, 'normal');

                            } else {
                                config.addSlides(response.data.Marketingslideticket, config.slides, parseInt(model.topage), 'normal');
                            }
                        } else {
                            if (from === 1) {
