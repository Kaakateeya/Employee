(function(angular) {
    ...
    function factory($http, searchpageServices, arrayConstants, SelectBindServiceApp, getArray, timeout,
        ...
        model.gridtable.columns = [
            ...
            { text: 'DOB(Age)', key: 'DOB(Age)', type: 'label', templateUrl: model.AgeTemplate },
            { text: 'Gender', key: 'Gender', type: 'label' },
            { text: 'Height', key: 'Height', type: 'label' },
            { text: 'EducationGroup', key: 'EducationGroup', type: 'label' },
            { text: 'Profession', key: 'Profession', type: 'label' },
            { text: 'joblocation', key: 'joblocation', type: 'label' },
            { text: 'countrylivingin', key: 'countrylivingin', type: 'label' },
            { text: 'Fathernative', key: 'Fathernative', type: 'label' },
            { text: 'Mothernative', key: 'Mothernative', type: 'label' }
        ];
        model.submitgeneral = function(object, frompage, topage, typeofexcel) {
            model.topage = topage;
            ...
            searchpageServices.generalsearchsubmit(model.CgetDetails).then(function(response) {
                ...
                if (model.typrofsearch === "2") {
                    ...
                    if (typeofexcel === 'excel') {
                        ...
                        alasql('SELECT ProfileID,FirstName,LastName as SurName,Caste,MaritalStatus,DOB(Age),Gender,Height,EducationGroup,Profession,joblocation,countrylivingin,Fathernative,Mothernative INTO  XLSX("john.xlsx",?) FROM ?', [options, model.grid.exportarray]);
                    } else {
                        model.opendiv = false;
        ...
        model.gridtable.pagechange = function(val) {
            var to = val * 10;
            var from = val === 1 ? 1 : to - 9;
            switch (model.tablename) {
                case "general":
                    model.submitgeneral(model.CgetDetails, from, to);
                    break;
                case "advanced":
                    model.submitadvancedsearch(model.CgetDetails, from, to);
                    break;
            }
        };
        model.gridtable.exportexcel = function(topage) {
            switch (model.tablename) {
                case "general":
                    model.submitgeneral(model.CgetDetails, 1, model.gridtable.TotalRows, 'excel');
                    break;
                case "advanced":
                    model.submitadvancedsearch(model.CgetDetails, 1, model.gridtable.TotalRows, 'excel');
                    break;
            }
        };
        ...
        model.submitadvancedsearch = function(object, frompage, topage, typeofexcel) {
            model.topage = topage;
            if (parseInt(frompage) === 1) {
                model.slides = [];
                model.CgetDetails.GetDetails = {
